    ...
    for config_item in split:
        ...
        if config_split:
            if config_split[0].startswith("refine"):
                use_refine = (
                    True if config_split[1].lower().startswith("true") else False
                )
...
def runNetworKitPLP(G, config):
    ...
    start_time = time.time()
    out, communities = capture_output(
        nk.community.detectCommunities,
        G,
        algo=nk.community.PLP(G, baseClustering=None, **kwargs),
    )
    end_time = time.time()
...
def runNetworKitLPDegreeOrdered(G, config):
    ...
    start_time = time.time()
    out, communities = capture_output(
        nk.community.detectCommunities, G, algo=nk.community.LPDegreeOrdered(G)
    )
    end_time = time.time()
    out += "\nCommunities detected in %f \n" % (end_time - start_time)
    return out, communities
    ...
    with redirect_stdout(f):
        ...
        communities = nk.community.detectCommunities(
            ...
            algo=nk.community.ParallelLeiden(
                G, randomize=use_randomize, iterations=use_iterations, gamma=use_gamma
            ),
        )
        end_time = time.time()
        print("Communities detected in %f \n" % (end_time - start_time))
...
def runNetworKitConnectivity(G, config):
    ...
    if G.isDirected():
        raise ValueError(
            "NetworkIt Connected Components can only run for undirected graphs."
        )
    with redirect_stdout(f):
        print(config)
        start_time = time.time()
    ...
    if cluster_config.postprocess_only != "true":
        ...
        if is_bin_extension(use_input_graph):
            ...
            reader = nk.graphio.EdgeListReader(
                "\t", 0, commentPrefix="#", directed=False
            )  # continuous=False,
        ...
        runner_utils.appendToFile(
            "Cluster Time: " + extractNetworKitTime(print_time) + "\n", out_filename
        )

        # Create an empty list to hold all the lines you want to write to the file
        if cluster_config.write_clustering != "false":
            ...
            if not cluster_flag:
                ...
                if use_original_networkit:
                    ...
                    #   lines_to_write.append("\t".join(str(x) for x in cluster_list))
                    nk.community.writeCommunitiesNestedFormat(
                        communities, out_clustering
                    )
            else:
